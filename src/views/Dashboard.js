...
const OPTION_MONTH_DAY = { day: "numeric", month: "long" };
const OPTION_MONTH_YEAR = { year: "numeric", month: "short", day: "numeric" };

const Dashboard = () => {
	const history = useSelector((state) => state.currencyHistory.history);
	...
	const currencies = useSelector((state) => state.currencyList.list);
	...
	const changeInterval = (interval) => {
		dispatch(
			getCurrencyHistory({ id: selected_currency_id, interval: interval })
		);
	};
	...
	const onSearchChange = (e) => {
		...
		let matched = currencies.filter((currency) => {
			const subString = currency.name
				.substr(0, e.target.value.length)
				.toLowerCase();
			return subString === e.target.value.toLowerCase();
		});
		if (matched.length > 5 && matched.length !== currencies.length) {
			matched = matched.slice(0, 5);
		} else if (matched.length == currencies.length) {
			matched = [];
		}
	...
	const selectSearchedCurrency = (currency) => {
		dispatch(getCurrencyHistory({ id: currency.id, interval: null }));
		setSearch("");
		setSuggestions([]);
	};
	...
	const generateLabels = () => {
		let date = null;
		let options =
			selectedInterval === "1D" ? OPTION_HOUR_MINUT : OPTION_MONTH_DAY;
		options = ["6M", "1Y"].includes(selectedInterval)
			? OPTION_MONTH_YEAR
			: options;
						...
						<ul className="suggestion-box">
							{suggestions.map((currency) => (
								<li
									key={currency.id}
									onClick={() => {
										selectSearchedCurrency(currency);
									}}>
									{currency.name}
								</li>
							))}
